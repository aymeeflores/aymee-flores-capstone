...
api.get("/feed", (req, res) => {
  let sql =
    "SELECT f.*, u.name, u.email, u.avatar FROM feeds f LEFT JOIN users u ON u.id = f.user_id WHERE f.location_id = ?";
  db.all(sql, [req.user.location_id], (err, row) => {
    if (err) {
      res.status(500).send(err.message);
      return;
...
api.put("/feed/:postid", (req, res) => {
  ...
  let sql =
    "UPDATE feeds SET image = ?, content = ? WHERE id = ? AND location_id = ? AND user_id = ?";
  let data = [image, content, postid, req.user.location_id, req.user.id];
...
api.delete("/feed/:postid", (req, res) => {
  ...
  let data = [req.params.postid, req.user.id];
  db.run(sql, data, function (err) {
    if (err) {
      res.status(500).send(err.message);
      return;
...
api.post("/events", (req, res) => {
  ...
  let data = [req.user.location_id, name, content, eventDate, date];
...
api.put("/events/:postid", (req, res) => {
  ...
  let sql =
    "UPDATE events SET name = ?, content = ?, date = ? WHERE id = ? AND location_id = ?";
  let data = [name, content, date, postid, req.user.location_id];
...
api.delete("/events/:postid", (req, res) => {
  ...
  let data = [req.params.postid, req.user.location_id];

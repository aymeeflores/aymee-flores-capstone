			...
			<>
				<div className="card feedform">
					<div className="card-body feedform__body">
						<div className="form-group feedinput">
							<input className="feedinput__input" type="text" placeholder={`Connect with your neighbors, ${window.user.name} `} id="inputLarge" onChange={(e) => this.setState({ comment: e.target.value })} />
						</div>
						<button type="button" className="sharebtn " onClick={this.createPost}>
							Share
						</button>
					</div>
				</div>
				...
				{this.state.posts.length > 0 &&
					this.state.posts.map((item, idx) => {
						...
						return <CardText item={item} deleteFnc={this.deletePost} key={idx} />;
					})}
			</>
		);
	}
